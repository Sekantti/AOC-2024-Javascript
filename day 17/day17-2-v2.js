...
const input = readFileSync('problem17-copy.in', 'utf-8');
...
function solve2(target, input, instructions) {
    let num = 0;

    while (true) {
        const result = solve(input, instructions)
        if (arrayEquals(result, target)) {
            return true;
        }
    }
}
...
//1, 5: B = 5^B === 5^(A&7)
//7, 5: C = A>>B === A>>(5^(A&7))
//4, 3: B = B^C === (5^(A&7))^(A>>(5^(A&7)))
//1, 6: B = 6^B === 6^((5^(A&7))^(A>>(5^(A&7))))
//0, 3: A = A>>3
//5, 5: output = B & 7 === (6^((5^(A&7))^(A>>(5^(A&7))))) & 7
//3, 0: start again.
